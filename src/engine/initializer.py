...
class SessionInitializer:
    ...
    async def initialize_session(
        ...
        logger.debug(f"Config: {num_characters} chars, world={generate_world}, entities={generate_entities}")
        ...
        try:
            ...
            if generate_entities and spec.entities:
                logger.debug(f"Generating backgrounds for up to {num_characters} entities...")
                ...
                logger.info(f"Generated backgrounds for {len(detailed_entities)} entities")
            else:
                detailed_entities = spec.entities
                logger.debug("Skipped entity background generation (disabled in config or no entities)")
...
Name: {context['name']}
Entity types: {', '.join(context['entities']) if context['entities'] else 'None yet'}
Key actions: {', '.join(context['actions'][:3])}
