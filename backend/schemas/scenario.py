...
class RandomEvent(BaseModel):
    ...
    when: Dict[str, Any] = Field(
        ..., description="JSONLogic condition for when event occurs"
    )
    effects: List[EffectOp] = Field(..., description="List of effects to apply")
    duration: Optional[str] = Field(
        None, description="Event duration (ISO-like format)"
    )
...
class NegativityBudget(BaseModel):
    ...
    min_fail_rate: float = Field(
        ..., gt=0.0, le=1.0, description="Minimum failure rate (must be > 0)"
    )
...
class ScenarioSpec(BaseModel):
    ...
    loss_conditions: List[LossCondition] = Field(
        ..., min_length=2, description="Loss conditions (≥2 required)"
    )
    negativity_budget: NegativityBudget = Field(
        ..., description="Negativity budget configuration"
    )
