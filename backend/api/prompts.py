...
async def enrich_prompt(request: PromptEnrichRequest):
    ...
    if not request.description or len(request.description.strip()) < 10:
        ...
        raise HTTPException(
            status_code=400, detail="Description must be at least 10 characters"
        )
    ...
    try:
        ...
        user_message = PROMPT_ENRICHMENT_USER.format(description=request.description)
        ...
        return PromptEnrichResponse(
            original=request.description, enriched=enriched_text
        )
